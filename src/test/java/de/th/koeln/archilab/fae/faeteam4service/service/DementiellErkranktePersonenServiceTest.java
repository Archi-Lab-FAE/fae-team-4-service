...
public class DementiellErkranktePersonenServiceTest {
  ...
  public void givenAlarmknopfAndDementiellErkranktePersonenRepoAndRadiusThenReturnPersonenInProximity_2_InRange() {
    ...
    List<DementiellErkranktePerson> personenInProximity = dementiellErkranktePersonenService
        .getPersonenInProximityOf(alarmknopf, radius);
  ...
  private Alarmknopf generateKnopfWithIdAndPosition(final String id, final double latitude,
      final double longitude) {
    Position position = getPositionFromLatitudeAndLongitude(latitude, longitude);
    return new Alarmknopf(id, position);
  }

  private DementiellErkranktePerson generatePersonWithPosition(final String trackerId,
